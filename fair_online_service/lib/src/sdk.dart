...
class Sdk {
  ...
  factory Sdk.create(String version) {
    final sdkPath = _resolveSdkPath(version);
    ...
    return _instance ??= Sdk._(
      ...
      flutterVersion: _readFlutterVersion(sdkPath),
    );
  }
  ...
  static String _readFlutterVersion(String sdkPath) {
    final flutterVersionFile = File(path.join(sdkPath, 'version'));
    if (flutterVersionFile.existsSync()) {
      return flutterVersionFile.readAsStringSync().trim();
    }
    return path.basename(sdkPath);
  }

  static String _resolveSdkPath(String version) {
    final projectSdkPath = path.join(_flutterSdksPath, version);
    if (_isCompleteSdk(projectSdkPath)) {
    ...
    for (final globalFvmSdkPath in _globalFvmSdkPaths(version)) {
      if (_isCompleteSdk(globalFvmSdkPath) &&
          _matchesExpectedVersion(globalFvmSdkPath, version)) {
        return globalFvmSdkPath;
      }
    }

    throw FileSystemException(
      'Flutter SDK not found or incomplete. '
      'Expected a complete SDK at "$projectSdkPath", or a matching FVM SDK at '
      '"$fvmSdkPath", or a matching global FVM SDK. You can run '
  ...
  static List<String> _globalFvmSdkPaths(String version) {
    ...
    final candidates = <String>{
      path.join(home, 'fvm', 'versions', version),
      path.join(home, '.fvm', 'versions', version),
    };

    final fvmrcVersion = _readFvmrcVersion();
    if (fvmrcVersion.isNotEmpty) {
      candidates.add(path.join(home, 'fvm', 'versions', fvmrcVersion));
      candidates.add(path.join(home, '.fvm', 'versions', fvmrcVersion));
    }
  ...
  static String _readFvmrcVersion() {
    final fvmrcFile = File(path.join(Directory.current.path, '.fvmrc'));
    if (!fvmrcFile.existsSync()) {
      return '';
    }

    try {
      final data = jsonDecode(fvmrcFile.readAsStringSync());
      if (data is Map<String, Object?>) {
        final flutterVersion = data['flutter'];
        if (flutterVersion is String) {
          return flutterVersion.trim();
        }
      }
    } catch (_) {
      return '';
  ...
  static bool _isCompleteSdk(String sdkPath) {
    final dartVersionFile =
        File(path.join(sdkPath, 'bin', 'cache', 'dart-sdk', 'version'));
    final flutterTool = File(path.join(sdkPath, 'bin', 'flutter'));
    return dartVersionFile.existsSync() && flutterTool.existsSync();
  }

  static bool _matchesExpectedVersion(String sdkPath, String expectedVersion) {
    final flutterVersionFile = File(path.join(sdkPath, 'version'));
    if (flutterVersionFile.existsSync()) {
      return flutterVersionFile.readAsStringSync().trim() == expectedVersion;
    }
    return path.basename(sdkPath) == expectedVersion;
  }
